...
def data_cleaner(dataset, feature_set_range, parent_labels, child_labels=None):
    ...
    # Check valid parent labels
    valid_parent_labels = [label for label in parent_labels if
                           label in dataset.columns and dataset[label].notna().sum() > 0]
    print(f"Valid parent labels: {valid_parent_labels}")

    # Handle missing parent labels
    ...
    X_parent = raw_data.iloc[:, :len(feature_set_range)].apply(pd.to_numeric, errors='coerce').fillna(0)
    y_parent = raw_data[valid_parent_labels].idxmax(axis=1)

    # Encode labels
    label_encoder = preprocessing.LabelEncoder()
    y_parent = label_encoder.fit_transform(y_parent)
...
if __name__ == "__main__":
    ...
    dataset_uuids = readdata_csv(data_dir)  # Replace `data_dir` with your actual data directory
    ...
    parent_labels = ['label:OR_standing', 'label:SITTING', 'label:LYING_DOWN', 'label:FIX_running', 'label:FIX_walking',
                     'label:BICYCLING']
    X_parent, y_parent = data_cleaner(dataset, feature_set_range, parent_labels)

    if X_parent is not None and y_parent is not None:
        # Split data
        X_train, X_test, y_train, y_test = train_test_split(X_parent, y_parent, test_size=0.2)
        ...
        model.fit(X_train, y_train, batch_size=32, epochs=50, validation_split=0.2, class_weight=class_weight_dict,
